  ...
  <v-col cols="12" md="6" class="DataCard MutantConfirmedCasesNumberCard">
    <client-only>
      <confirmed-cases-number-chart
        :title="$t('公表日別による変異株（アルファ株）陽性者数の推移')"
        :title-id="'number-of-mutant-alpha-confirmed-cases'"
        :chart-id="'number-of-mutant-alpha-confirmed-cases-chart'"
        ...
        :url="'https://a01sa01to.com/opendata/covid19-ibaraki/mutant-positive-number/'"
      >
...
export default {
  ...
  data() {
    const patientsGraph = formatGraph(
      Data.mutant_summary.data.filter((d) => d.name === 'Alpha')
    )
    const date = '2021-06-14 23:59'
