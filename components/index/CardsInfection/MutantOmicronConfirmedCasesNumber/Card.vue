  ...
  <v-col cols="12" md="6" class="DataCard MutantConfirmedCasesNumberCard">
    <client-only>
      <confirmed-cases-number-chart
        :title="$t('公表日別による変異株（オミクロン株）陽性者数の推移')"
        :title-id="'number-of-mutant-omicron-confirmed-cases'"
        :chart-id="'number-of-mutant-omicron-confirmed-cases-chart'"
...
export default {
  ...
  data() {
    const patientsGraph = formatGraph(
      Data.data.filter((d) => d.name === 'Omicron')
    )
