<template>
  <v-col cols="12" md="6" class="DataCard VaccinationCard">
    <client-only>
      <chart
        :title="$t('ワクチン接種回数（6回目）')"
        title-id="vaccination-6th"
        chart-id="vaccination-6th-chart"
        ...
        <template #description>
          <ul>
            ...
            <li>
              {{
                $t(
                  '接種率は、県全体の人口から医療従事者数を引いた数（約283万人）に対する割合である。人口は2023年4月1日現在のもの。'
                )
              }}
            </li>
            <li>
...
export default Vue.extend({
  ...
  data() {
    ...
    const summaryGraph = formatGraph(
      VaccineData.filter((_) => new Date(_.date) >= new Date('2023-05-01')).map(
        (_) => ({ date: _.date, total: _.status_6 })
      )
    )
