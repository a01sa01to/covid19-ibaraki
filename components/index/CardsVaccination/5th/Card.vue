<template>
  <v-col cols="12" md="6" class="DataCard VaccinationCard">
    <client-only>
      <chart
        :title="$t('ワクチン接種回数（5回目）')"
        title-id="vaccination-5th"
        chart-id="vaccination-5th-chart"
...
export default Vue.extend({
  ...
  data() {
    ...
    const summaryGraph = formatGraph(
      VaccineData.filter((_) => new Date(_.date) >= new Date('2022-10-15')).map(
        (_) => ({ date: _.date, total: _.status_5 })
      )
    )
