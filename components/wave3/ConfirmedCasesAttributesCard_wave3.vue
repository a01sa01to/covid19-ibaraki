    ...
    <client-only>
      ...
      >
        ...
        <template #description>
          <ul>
            <li>
              {{ $t('997-1054例目・1491-1500例目・2801-2880例目・3938-3954例目の方については、概要のみが公表されているため、年代・性別が異なっている可能性がある') }}
            </li>
          </ul>
        </template>
      </data-table>
...
> = {
  ...
  methods: {
    async fetchOpenAPI() {
      ...
      const url = endpoint + encodeURIComponent(`num__No__under__${this.endCursor+723};num__No__over__${lastIndividualNum < 1 ? 724 : lastIndividualNum+723}`)
