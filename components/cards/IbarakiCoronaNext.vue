        ...
        <template #button>
          <ul :class="$style.note">
            <li>
              {{
                $t(
                  '医療提供体制については、空港検疫や県外で確認された陽性者による病床利用を算入していることなどから、「茨城県内の新型コロナウイルス感染症の陽性者の状況」の入院者数とは一致しない。'
                )
              }}
            </li>
          </ul>
...
export default {
  ...
  data() {
    ...
    const _Y = {
      ...
      new_patients: patients.new_patientsYesterday,
      non_closecontact: patients.non_closecontactYesterday,
    }
    const delta = {}
    for (const key in _) {
